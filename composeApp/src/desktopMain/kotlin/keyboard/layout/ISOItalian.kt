...
enum class ISOItalian(
	override val keyCode: Int,
	override val kc: KC,
	override val shift: KC? = null,
	override val altGr: KC? = null,
	override val shiftAltGr: KC? = null,
) : Key {
	...
	UNKNOWN(0x00, KC.UNKNOWN)
	;
	
	// TODO: find a way to do this on the interface or something
	companion object {
		/**
		 * Get the key from the key code, if it does not exist, return UNKNOWN.
		 *
		 * @param keyCode The key code to look up.
		 * @return The corresponding Key, or UNKNOWN if not found.
		 */
		fun getKey(keyCode: Int): Key = entries.find { it.keyCode == keyCode } ?: UNKNOWN
		 ...
		 * @param altGr Whether the altGr key is pressed.
		 * @return The corresponding KC based on the modifiers.
		 */
