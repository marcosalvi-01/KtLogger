...
class WindowInfo(
	...
	fun addKeyPress(key: KC) {
		...
		// Update the bigram
		currentBigram = currentBigram.second to key
		currentBigram.let { (first, second) ->
			if (first != null && second != null) {
				bigrams[first to second] = bigrams.getOrDefault(first to second, 0) + 1
			}
		}
		
		// Update the trigram
		currentTrigram = Triple(currentTrigram.second, currentTrigram.third, key)
		currentTrigram.let { (first, second, third) ->
			if (first != null && second != null && third != null) {
				trigrams[Triple(first, second, third)] =
					trigrams.getOrDefault(Triple(first, second, third), 0) + 1
			}
		}
	}
